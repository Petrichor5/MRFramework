...
namespace MRFramework
{
    public class SystemUIEditor : Editor
    {
        ...
        [MenuItem("GameObject/UIPanel（MRFramework）/自动生成UI脚本(Shift+Z) #Z", false, 0)]
        private static void CreateAutoGenCodeScript()
        {
            AutoGenCode.CodeGen();
        }

        [MenuItem("GameObject/UIPanel（MRFramework）/一键优化合批(Shift+X) #X", false, 1)]
        [Obsolete("Obsolete")]
        ...
        [MenuItem("GameObject/UIPanel（MRFramework）/创建 UIRoot 拼接窗口(Shift+C) #C", false, 2)]
        ...
        [MenuItem("GameObject/UIPanel（MRFramework）/创建一级窗口(Shift+1) #1", false, 3)]
        public static void BuildPanel1()
        {
            InitBuildPanel("TempPanel1");
        }
        
        [MenuItem("GameObject/UIPanel（MRFramework）/创建二级窗口(Shift+2) #2", false, 4)]
        public static void BuildPanel2()
        {
            InitBuildPanel("TempPanel2");
        }
        
        [MenuItem("GameObject/UIPanel（MRFramework）/创建三级窗口(Shift+3) #3", false, 5)]
        public static void BuildPanel3()
        {
            InitBuildPanel("TempPanel3");
        }
        
        [MenuItem("GameObject/UIPanel（MRFramework）/创建四级窗口(Shift+4) #4", false, 6)]
        public static void BuildPanel4()
        {
            InitBuildPanel("TempPanel4");
        }
        
        [MenuItem("GameObject/UIPanel（MRFramework）/创建五级窗口(Shift+5) #5", false, 7)]
        public static void BuildPanel5()
        {
            InitBuildPanel("TempPanel5");
        }
