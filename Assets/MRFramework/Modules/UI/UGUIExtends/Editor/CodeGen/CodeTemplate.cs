...
namespace MRFramework
{
    ...
    public static class CodeTemplate
    {
        ...
        private static void CreatPanelScript(ref CodeGenTask task)
        {
            ...
            // OnDestroy
            writer.AppendLine("\tpublic override void OnDestroy()");
            writer.AppendLine("\t{");
            writer.AppendLine("\t\tbase.OnDestroy();");
            writer.AppendLine("\t}");
            writer.AppendLine();
        ...
        private static void InitScrollView(StringBuilder writer)
        {
            ...
            foreach (var bindData in m_Task.Binds)
            {
                ...
                {
                    string memberName = bindData.MemberName;
                    writer.AppendLine($"\t\tAddScrollView({memberName});");
                    hasWidget = true;
                }
            }
