...
public class DataManager : Singleton<DataManager>
{
    ...
    public async UniTask SaveDataAsync(object data, string dir, string fileName, ESaveType saveType)
    {
    ...
    /// <summary>
    /// 异步读取 Json 文件并反序列化为对象
    /// </summary>
    /// <param name="dir">目录</param>
    /// <param name="fileName">Json 文件名</param>
    /// <param name="saveType">存储类型</param>
    public async UniTask<T> LoadDataAsync<T>(string dir, string fileName, ESaveType saveType) where T : new()
    {
        string folderPath = GetSavePath(dir, saveType);
    ...
    /// <param name="fileName">Json 文件名</param>
    /// <param name="saveType">存储类型</param>
    public void SaveData(object data, string dir, string fileName, ESaveType saveType)
    {
        string folderPath = GetSavePath(dir, saveType);
        folderPath.CreateDirIfNotExists();
    ...
    /// <param name="fileName">Json 文件名</param>
    /// <param name="saveType">存储类型</param>
    public T LoadData<T>(string dir, string fileName, ESaveType saveType) where T : new()
    {
        string folderPath = GetSavePath(dir, saveType);
        if (!Directory.Exists(folderPath))
        {
    ...
    private string GetSavePath(string dir, ESaveType saveType)
    {
        // PersistentDataPath 游戏打包后该文件夹在任何平台都可读可写
        string rootPath = string.Empty;
        switch (saveType)
        {
            case ESaveType.PersistentData:
                rootPath = Application.persistentDataPath;
                break;
            case ESaveType.StreamingAssetsPath:
    ...
    public void SavePlayerPrefsData(object data, string keyName)
    {
    ...
    /// <param name="keyName">数据对象的唯一 Key</param>
    public object LoadPlayerPrefsData(Type dataType, string keyName)
    {
