...
namespace MRFramework.UGUIPro
{
    ...
    /// UGUI描边
    /// </summary>
    public class TextProOutLine : UnityEngine.EventSystems.UIBehaviour
    {
        ...
        [SerializeField, Range(0, 10)] [HideInInspector]
        ...
        private void _ProcessVertices2()
        {
            ...
            for (int i = 0, count = m_VetexList.Count - 6; i <= count; i += 6)
            {
                ...
                // 计算原顶点坐标中心点
                //
                var minX = _Min(v1.position.x, v2.position.x, v3.position.x, v4.position.x, v5.position.x,
                    v6.position.x);
                var minY = _Min(v1.position.y, v2.position.y, v3.position.y, v4.position.y, v5.position.y,
                    v6.position.y);
                var maxX = _Max(v1.position.x, v2.position.x, v3.position.x, v4.position.x, v5.position.x,
                    v6.position.x);
                var maxY = _Max(v1.position.y, v2.position.y, v3.position.y, v4.position.y, v5.position.y,
                    v6.position.y);
        ...
        private static UIVertex _SetNewPosAndUV(UIVertex pVertex, float pOutLineWidth, Vector2 pPosCenter,
            Vector2 pTriangleX, Vector2 pTriangleY,
            Vector2 pUVX, Vector2 pUVY, Vector2 pUVOriginMin, Vector2 pUVOriginMax, bool needOffset = true,
            string name = "")
        {
            // Position
            ...
            vx += pUVX / Mathf.Min(pTriangleX.magnitude, 18) * posXOffset *
                  (Vector2.Dot(pTriangleX, Vector2.right) > 0 ? 1 : -1);
            vx += pUVY / Mathf.Min(pTriangleY.magnitude, 18) * posYOffset *
