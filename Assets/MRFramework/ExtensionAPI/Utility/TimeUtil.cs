...
{
    private static StringBuilder mResultStr = new StringBuilder();
        ...
        string minuteStr = "分", string secondStr = "秒")
    {
        ...
        if (hour != 0 || !egZero)
        {
            mResultStr.Append(isKeepLen ? StringUtil.GetNumStr(hour, 2) : hour); //具体几个小时
            mResultStr.Append(hourStr);
        }

        //如果分钟不为0 或者 不忽略0 或者 小时不为0
        if (minute != 0 || !egZero || hour != 0)
        {
            mResultStr.Append(isKeepLen ? StringUtil.GetNumStr(minute, 2) : minute); //具体几分钟
            mResultStr.Append(minuteStr);
        }

        //如果秒不为0 或者 不忽略0 或者 小时和分钟不为0
        if (second != 0 || !egZero || hour != 0 || minute != 0)
        {
            mResultStr.Append(isKeepLen ? StringUtil.GetNumStr(second, 2) : second); //具体多少秒
            mResultStr.Append(secondStr);
        }
